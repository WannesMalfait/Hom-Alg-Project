...
We can say more:
\begin{corollary}
    The functor $(-)^\sharp \PSh(\mcC) \to \Sh(\mcC)$ is exact.
\end{corollary}
\begin{proof}
    ...
    such that $s_i |_{U_j} = s_{ij}$. In order for $(s_i)_{i\in \mcI}$ to be
    an element of $\mcF(U)$ we need that for $f\colon i\to i'$ in $\mcI$,
    $\mcF(f)(s_i) = s_{i'}$. This follows, as $s_i|_{U_j}$ is mapped to $s_{i'}|_{U_j}$
    for all $j\in J$, and hence $s_i$ is mapped to $s_{i'}$ because
    $\mcF_{i'}$ is a sheaf. We have $s = (s_i)_{i\in \mcI} \in \mcF(U)$,
    and by the universal property of the limit
    \begin{equation*}
        s|_{U_j} = s_j \iff s_i|_{U_j} = s_{ij}\; \forall j\in J,
    \end{equation*}
