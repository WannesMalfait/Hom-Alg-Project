...
One defines a \emph{sieve} on an object $U \in \mcC$ as a subfunctor
of its Yoneda embedding $\Hom(- ,U)$. So, it is a functor $S \colon \mcC^{\opp} \to \mcC$
such that for any $U' \in \mcC$ we have $S(U') \subseteq \Hom(U', U)$,
and for all morphisms $f\colon V \to V'$, $S(f)$ is the restriction
of $\Hom(f, U)$ to $S(V')$. A site becomes a category together with
...
\begin{enumerate}
    \item If $S$ is a covering sieve on $X$
    TODO: axioms
\end{enumerate} 
TODO: explain what sheaf is in this context.
TODO: explain what site of extremally disconnected becomes.
TODO: how to get sieves from covers.
