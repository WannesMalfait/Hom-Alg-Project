...
\begin{lemma}
    ...
    map $\mcF(\mcV) \to \mcF(\mcU)$. Since $\{1_U\}$ is a cover
    by the axioms of a site, this gives a map $\mcF(U) \cong \mcF(\{1_U\}) \to \mcF(\mcU)$.
    % This extends $\mcF$ to a functor $\mcF \colon \Cov(U)^{\opp} \to \Set$,
    % in the sense that $\mcF(\{1_U\}) = \mcF(U)$.
...
\begin{proof}
    ...
    The last claim is just the definition of sheaves on a site.
\end{proof}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \end{tikzcd}
    \end{equation*}
    TODO: show functoriality of $\mcF^+$ with $U \to V \to W$
\end{proof}
